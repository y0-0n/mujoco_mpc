...
namespace mjpc::common_rig {
std::string Motion::XmlPath() const {
  return GetModelPath("common_rig/motion/task.xml");
}
std::string Motion::Name() const { return "Common Rig Motion"; }
...
void Motion::ResidualFn::Residual(const mjModel* model, const mjData* data,
  ...
  int counter = 0;
  int tick = int(data->time / 0.05) % 50;
